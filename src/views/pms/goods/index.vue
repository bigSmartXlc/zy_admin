...
function handleQuery() {
  ...
  listSpuPages(state.value.queryParams).then(({ data }) => {
    state.value.goodsList = data.list;
...
function handleDelete(row: any) {
  const ids = row.id || state.value.ids.join(',');
...
function handleSelectionChange(selection: any) {
  state.value.ids = selection.map((item: { id: any }) => item.id);
